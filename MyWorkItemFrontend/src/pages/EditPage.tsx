...
export function EditPage() {
  ...
  useEffect(() => {
    if (!id) return;
    workItemApi.getById(Number(id))
      .then(setItem)
      .catch(() => setError('找不到此工作項目'))
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-2">編輯工作項目</h1>
          <p className="text-sm text-gray-500 mt-1">#{item.id} · {item.title}</p>
        </div>
